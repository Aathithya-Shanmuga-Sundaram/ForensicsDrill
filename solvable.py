#!/usr/bin/env python3
"""
ForensicsDrill - Full IR Simulation (Mode C) - FLAG-EMBEDDED EDITION
(Replace your previous file with this one.)

Usage:
  python forensics_drill.py init --session lab1 [--seed 123] [--scenario ransomware_campaign]
...
# ----------------- Flag embedding helper -----------------
def embed_flag(ws: Workspace, flag: str, method: str, relpath: str):
    """
    Embed the flag into a workspace file at relpath with encoding method.
    method: "plain", "base64", "hex", "rot13"
    """
    full = os.path.join(ws.root, relpath)
    ensure_dir(os.path.dirname(full))
    content = ""
    if method == "plain":
        content = f"{flag}\n"
    elif method == "base64":
        content = base64.b64encode(flag.encode()).decode() + "\n"
    elif method == "hex":
        content = binascii.hexlify(flag.encode()).decode() + "\n"
    elif method == "rot13":
        content = codecs_encode_rot13(flag) + "\n"
    else:
        content = flag + "\n"
    with open(full, "w") as f:
        f.write(content)
    # set a slightly older mtime for realism
    ts = (datetime.utcnow() - timedelta(days=random.randint(1,30))).timestamp()
    os.utime(full, (ts, ts))

def codecs_encode_rot13(s):
    # lightweight rot13 (no external imports)
...
def create_ransom_files(ws: Workspace, flag=None):
    ...
    # embed flag encoded (base64) inside ransom note as a 'serial' string
    if flag:
        with open(note, "a") as nf:
            nf.write("\nRecovery-key (base64): " + base64.b64encode(flag.encode()).decode() + "\n")
    # also place a hidden .flag file
    if flag:
        embed_flag(ws, flag, "plain", "disk/.recovery_key.flag")
    return note

def create_staged_data(ws: Workspace, flag=None):
    ...
    for i in range(3):
        ...
        with open(p, "w") as f:
            f.write("id,value\n")
            for j in range(200):
                # occasionally insert a fragment of base64-encoded flag into staged files
                if flag and i == 1 and j == 5:
                    f.write(f"{j},{base64.b64encode(flag.encode()).decode()}\n")
                else:
...
def create_persistent_task(ws: Workspace, flag=None):
    p = os.path.join(ws.disk, "task_malicious.schtask")
    with open(p, "w") as f:
        f.write("schtasks /create /sc onlogon /tn MalService /tr C:\\mal\\persistsvc.exe\n")
    if flag:
        # embed flag hex inside task file as 'id' field
        with open(p, "a") as f:
            f.write("TaskID: " + binascii.hexlify(flag.encode()).decode() + "\n")
    return p
...
class DrillEngine:
    ...
    def init_workspace(self):
        # remove only expected generated content for safety
        ...
        # scenario-specific extras with flag embedding
        if self.scenario_key == "ransomware_campaign":
            create_ransom_files(self.ws, flag=self.flag)
        elif self.scenario_key == "credential_theft_exfil":
            create_staged_data(self.ws, flag=self.flag)
            with open(os.path.join(self.ws.disk, "creds_dump.txt"), "w") as f:
                f.write("user1:password123\nadmin:adminpass\n")
                # append flag plainly so investigators can find it by search
                f.write("\n# leaked-data: " + self.flag + "\n")
            ...
            with open(os.path.join(self.ws.disk, "run_on_startup.lnk"), "w") as f:
                ...
                f.write("ShortcutID: " + self.flag + "\n")
            create_persistent_task(self.ws, flag=self.flag)
        # write processes
    ...
    def investigate(self, target):
        ...
        try:
            ...
            try:
                ...
                # reveal flag if present in the sample
                if self.flag in txt:
    ...
    def submit_flag(self, flag):
        ...
        s = flag.strip()
        canonical = s
        # accept without braces, case-insensitive
        if canonical.lower().startswith("fd{") and canonical.endswith("}"):
            canonical = canonical
        else:
            # try add braces if missing
            if not (canonical.startswith("FD{") or canonical.startswith("fd{")):
        ...
        if canonical.strip() == self.flag:
            ...
            return True
        # also accept exact match ignoring case
    ...
    def provide_guide(self):
        ...
        if self.scenario_key == "ransomware_campaign":
            print(" 1) Explore disk/ and logs/; look for .locked files and README_RESTORE_FILES.txt")
            print(" 2) Inspect processes.json for suspicious svch0st.exe/cmd chains")
            print(" 3) Run ioc-scan to find ransom_note & encrypted files; check disk/.recovery_key.flag")
            print(f" FLAG: {self.flag}")
        elif self.scenario_key == "credential_theft_exfil":
            print(" 1) Search for creds_dump or staged files in disk/staged/")
            print(" 2) Inspect network.log for outbound connections")
            print(" 3) Check staged files for base64 fragments and creds_dump.txt")
            print(f" FLAG: {self.flag}")
        elif self.scenario_key == "persistence_startup":
            print(" 1) Look for scheduled tasks / run_on_startup.lnk and inspect them")
    ...
    def hint(self):
        hints = {
            "ransomware_campaign": "Look for README_RESTORE_FILES.txt in disk/ and check for base64-looking strings or disk/.recovery_key.flag",
            "credential_theft_exfil": "Search for 'creds' or check disk/staged and network.log; staged files may contain base64 fragments",
            "persistence_startup": "Check run_on_startup.lnk and task_malicious.schtask for odd IDs (hex or flag-like values)",
        }
...
class Shell:
    ...
    def run(self):
        print(f"{BOLD}{BLUE}[FORENSICS DRILL v1.1 - FLAG EMBEDDED]{RESET}\n{info('Type `help` for commands. Ctrl+C to exit.')}") 
        while True:
            try:
                cmdline = input("\n> ").strip()
...
# ----------------- Main Entrypoint -----------------
def main():
    ap = argparse.ArgumentParser(description="ForensicsDrill - Full IR Simulation (flag-embedded)")
